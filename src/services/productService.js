...
exports.getAllProductsService = async (filters) => {
  ...
  // 💰 4. Price range logic (Modified for Variants)
  // $elemMatch ব্যবহার করা হয়েছে যাতে অন্তত একটি ভেরিয়েন্ট রেঞ্জের মধ্যে থাকে
  if (minPrice || maxPrice) {
    query.variants = {
      $elemMatch: {
        price: {
  ...
  // 📦 5. Stock status logic (Modified)
  if (stock) {
    query["variants.stockStatus"] = stock;
  }

  // 🔢 6. Pagination Logic
  const resPerPage = Number(limit) || 12;
  const skip = (Number(page) - 1) * resPerPage;

  // 🚀 7. Sorting Logic (Modified)
  let sortBy = { createdAt: -1 };
  // 'variants.0.price' মানে হলো প্রথম ভেরিয়েন্টের দাম অনুযায়ী সর্ট হবে
  if (sort === "priceLow") sortBy = { "variants.0.price": 1 };
  if (sort === "priceHigh") sortBy = { "variants.0.price": -1 };
